...
require get_theme_file_path('/inc/posts-functions.php');
...
/*Theme Files*/
  function tha_files() {
    ...
    wp_enqueue_script('dompurify', get_theme_file_uri('/js/purify.min.js'), NULL, time());
  ...
  function custom_pagination(){
    echo paginate_links(
      array(
        ...
        'end_size' => 2,
        'mid_size' => 2,
        'prev_text'=> __('<span class="pagination-prevnext">⮜</span>'),
        'next_text'=> __('<span class="pagination-prevnext">⮞</span>')
      )
    );
  };
...
/* Shorthand numbers */
function shorthandNumber($x){
  if ($x > 999 AND $x <= 9999){
    $x = number_format($x/1000, 1) . 'K';
  }
  if ($x > 9999 AND $x <= 999999){
    $x = number_format($x/1000, 0) . 'K';
  }
  if ($x > 999999 AND $x <= 99999999){
    $x = number_format($x/1000000, 1) . 'M';
  }
  if ($x > 99999999 AND $x <= 999999999){
    $x = number_format($x/1000000, 0) . 'M';
  }
  return $x;
}